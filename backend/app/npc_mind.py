...
async def update_minds(llm, npcs, present, action, narrative, player_attrs):
    """更新在场 NPC 心智。返回 (npc_updates, plot_advanced, attr_changes, item_changes)。
    ...
    sections = "\n\n".join(npc_section(n, npcs[n], "本回合他在场，亲历了玩家行动与剧情") for n in names)
    try:
        raw = await llm.chat(
            [{"role": "system", "content": prompts.NPC_MIND_SYSTEM},
             {"role": "user", "content": prompts.npc_mind_user_message(action, narrative, sections)}],
    ...
    return updates, plot_advanced, attr_changes, {"add": add, "remove": remove}
