...
async def update_minds(llm, npcs, present, action, narrative, player_attrs,
                       main_plot="", knowledge_by_npc=None, npc_sections=None):
    """更新在场 NPC 心智。

    返回 (npc_updates, new_npcs, plot_advanced, main_plot_update,
    attr_changes, item_changes)。每个 NPC 只收到自己的目击历史窗口。
    ...
    for name, fields in (obj.get("npcs") or {}).items():
        ...
        if upd:
            updates[name] = upd
    unknown_names = [n for n in present if n not in npcs][:C.MAX_NPC_UPDATE]
    new_npcs = {}
    for card in (obj.get("new_npcs") or []):
    ...
    return updates, new_npcs, plot_advanced, main_plot_update, attr_changes, {"add": add, "remove": remove}
